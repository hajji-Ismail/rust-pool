...
impl FormError {
    pub fn new(field_name: &'static str, field_value: String, err: &'static str) -> Self {
        FormError{form_values :(field_name.to_string(), field_value) , err: err.to_string() , date : chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string()}
    ...
    pub fn validate(&self) -> Result<(), FormError> {
        ...
        if self.name.len() == 0 {
            return Err(FormError {
                ...
                form_values: ("name".to_string(), self.name.to_string()),
                date:data,
            });
        }
        if self.password.len() < 8 {
            return Err(FormError {
                ...
                form_values: ("password".to_string(), self.password.to_string()),
                date:data,
            });
        }
...
if !(digit&&letter&& symbol){
        return Err(FormError {
                err: "Password should be a combination of ASCII numbers, letters and symbols".to_string(),
                form_values: ("password".to_string(), self.password.to_string()),
                date: data,
            });
}
